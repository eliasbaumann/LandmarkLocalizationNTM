...
class Encoder_Decoder_Wrapper(tf.keras.layers.Layer):
    def __init__(self, num_filters, kernel_size, pool_size, batch_size):
        ...
        self.conv = [tf.keras.layers.Conv2D(filters=num_filters, kernel_size=self.kernel_size, activation='relu', padding='same', data_format='channels_first') for _ in range(5)]
        ...
        self.ds = tf.keras.layers.AveragePooling2D(pool_size=self.pool_size, strides=self.pool_size, data_format='channels_first')
        self.us = tf.keras.layers.UpSampling2D(size=self.pool_size, data_format='channels_first')
