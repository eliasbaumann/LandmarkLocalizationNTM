...
class Encoder_Decoder_Wrapper(tf.keras.layers.AbstractRNNCell):
    ...
    def __init__(self, ntm_config, batch_size,name='enc_dec', **kwargs):
        ...
        self.conv_out = tf.keras.layers.Conv2D(filters=1, kernel_size=self.kernel_size, activation=tf.nn.leaky_relu, padding='same', data_format='channels_first', name=self.name+'enc_dec_out')
